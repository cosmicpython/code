...
class TestChangeBatchQuantity:
    ...
    def test_changes_available_quantity():
        uow = FakeUnitOfWork()
        messagebus.handle([events.BatchCreated("b1", "ADORABLE-SETTEE", 100, None)], uow)
    ...
    def test_reallocates_if_necessary():
        uow = FakeUnitOfWork()
        messagebus.handle([
            events.BatchCreated("b1", "INDIFFERENT-TABLE", 50, None),
            events.BatchCreated("b2", "INDIFFERENT-TABLE", 50, date.today()),
            events.AllocationRequest("o1", "INDIFFERENT-TABLE", 20),
            events.AllocationRequest("o2", "INDIFFERENT-TABLE", 20),
        ], uow)
